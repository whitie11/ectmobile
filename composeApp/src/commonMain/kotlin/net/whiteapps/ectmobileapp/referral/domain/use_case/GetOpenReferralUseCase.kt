...
class GetOpenReferralUseCase (
    private val refRepository: ReferralRepository,
) {
    operator fun invoke(suId: Int): Flow<Resource<ReferralServicerUserResponseDTO>> = flow {
        try {
            emit(Resource.Loading())
            when(val res = refRepository.getOpenReferral(suId)) {
                is AppResult.Error ->   {
